...
public class PicassoUtils {
    ...
    public static void loadImageViewSize(Context mContext, String url, int width, int height, ImageView imageView) {
        if (url.trim().length() != 0){
            Picasso.with(mContext).load(url).config(Bitmap.Config.RGB_565).placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher).resize(width, height).centerCrop().into(imageView);
        }
    }
